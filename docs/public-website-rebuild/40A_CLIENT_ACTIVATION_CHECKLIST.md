...
| **Credit limits** | **Every account reads NULL** — meaning *not configured*, never unlimited, never zero. No numerical limits were invented. Setting one now requires the `finance.credit_limit` capability (which nobody holds) through `PUT /admin/credit/customers/:id/credit-limit`, with a reason, recorded in the append-only audit log. **The admin screen now exists** (Finance → Collection & Debt → Credit), and is unusable until the capability is granted. See `47_CREDIT_OPERATIONS.md` |
| **Credit approval workflow** | **BUILT.** Finance → Credit Reviews lists orders held above their limit, with approve / decline behind `finance.credit_review` — a SEPARATE capability from setting limits, because approving one order must never imply raising the ceiling. Both decisions need a reason and are audited. **Nobody holds the capability**, so grant it to the right people before this can be used. Approving an exception never changes a credit limit |
