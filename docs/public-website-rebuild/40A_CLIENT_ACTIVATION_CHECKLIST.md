...
| **Credit limits** | **Every account reads NULL** — meaning *not configured*, never unlimited, never zero. No numerical limits were invented. Setting one now requires the `finance.credit_limit` capability (which nobody holds) through `PUT /admin/credit/customers/:id/credit-limit`, with a reason, recorded in the append-only audit log. **The admin screen for it does not exist yet** — the endpoint is the mechanism. See `46_COMMERCIAL_CREDIT_AND_PRIVACY.md` |
| **Credit approval workflow** | **Decision required.** The server now detects when an order would take an account over its configured limit, records the figures, and flags the order — it does NOT reject it. Resolving that flag has deliberately no capability, no route and no screen: who may approve an over-limit order, and what approval means commercially, are Veyora's decisions to make. Orders awaiting one are `orders.credit_review is not null` |
